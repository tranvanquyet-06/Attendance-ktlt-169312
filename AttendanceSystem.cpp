...
bool HeThongDiemDanh::daTonTai(const DynamicArray<string> &ds,
                               const string &val) {
...
void HeThongDiemDanh::themLopHoc() {
  ...
  cout << endl
       << "  [OK] Them lop hoc \"" << lh.maLop << "\" thanh cong!" << endl;
  nhanEnter();
}
...
void HeThongDiemDanh::suaLopHoc() {
  ...
  getline(cin, input);
  if (!input.empty())
    dsLopHoc[idx].tenLop = input;

  cout << "  Thu hoc moi: ";
  getline(cin, input);
  if (!input.empty())
    dsLopHoc[idx].thu = input;
...
void HeThongDiemDanh::xoaLopHoc() {
  ...
  if (!svTrongLop.empty()) {
    cout << "  [CANH BAO] Lop nay con " << svTrongLop.size() << " sinh vien!"
         << endl;
  }

  char xacNhan;
  xacNhan =
      docXacNhan("  Ban co chac chan muon xoa lop \"" + maLop + "\"? (Y/N): ");
...
void HeThongDiemDanh::hienThiDanhSachLop() {
  ...
  for (int i = 0; i < dsLopHoc.size(); i++) {
    DynamicArray<int> svLop = laySVTheoLop(dsLopHoc[i].maLop);
...
void HeThongDiemDanh::xoaSinhVien() {
  ...
  char xacNhan;
  xacNhan = docXacNhan("  Ban co chac chan muon xoa? (Y/N): ");
...
void HeThongDiemDanh::hienThiDanhSachSV() {
  ...
  cout << endl;
  cout << "  " << left << setw(5) << "STT" << setw(15) << "Ma SV" << setw(30)
       << "Ho ten" << setw(15) << "Ma lop" << endl;
  cout << "  ";
  for (int i = 0; i < 63; i++)
    cout << "-";
  cout << endl;
...
void HeThongDiemDanh::diemDanhTheoLop() {
  ...
  if (daDiemDanh) {
    ...
    char xn;
    xn = docXacNhan("  Ban co muon diem danh lai? (Y/N): ");
    if (xn != 'Y' && xn != 'y')
      return;
  ...
  cout << endl;
  cout << "  Huong dan: 1 = Co mat | 2 = Vang co phep | 3 = Vang khong phep"
       << endl;
  ...
  cout << endl
       << "  [OK] Diem danh thanh cong cho " << svTrongLop.size()
       << " sinh vien!" << endl;
  nhanEnter();
}
...
void HeThongDiemDanh::suaDiemDanh() {
  ...
  string maLop, maSV, ngay;
  maLop = docChuoiKhongRong("  Nhap ma lop: ");
  maSV = docChuoiKhongRong("  Nhap ma sinh vien: ");
  ngay = docNgayHopLe("  Nhap ngay (dd/mm/yyyy): ");
...
void HeThongDiemDanh::timKiemTheoNgay() {
  ...
  cout << endl;
  cout << "  Ket qua diem danh lop " << maLop << " ngay " << ngay << ":"
       << endl;
  cout << "  " << left << setw(5) << "STT" << setw(15) << "Ma SV" << setw(30)
       << "Ho ten" << setw(20) << "Trang thai" << endl;
  cout << "  ";
  for (int i = 0; i < 68; i++)
    cout << "-";
  cout << endl;
...
void HeThongDiemDanh::timKiemTheoMaSV() {
  ...
  cout << endl
       << "  Sinh vien: " << dsSinhVien[svIdx].hoTen
       << " | Lop: " << dsSinhVien[svIdx].maLop << endl;
  cout << endl;

  cout << "  " << left << setw(5) << "STT" << setw(15) << "Ma lop" << setw(15)
       << "Ngay" << setw(20) << "Trang thai" << endl;
  cout << "  ";
  for (int i = 0; i < 53; i++)
    cout << "-";
  cout << endl;
  ...
  if (dem == 0) {
    ...
    for (int i = 0; i < dsDiemDanh.size(); i++) {
      if (dsDiemDanh[i].maSV == maSV) {
        if (dsDiemDanh[i].trangThai == CO_MAT)
          coMat++;
        else if (dsDiemDanh[i].trangThai == VANG_CO_PHEP)
          vangCP++;
        else if (dsDiemDanh[i].trangThai == VANG_KHONG_PHEP)
          vangKP++;
      }
    }
    cout << endl
         << "  Tong ket: Co mat: " << coMat << " | Vang CP: " << vangCP
...
void HeThongDiemDanh::tinhTyLeVang() {
  ...
  cout << endl
       << "  Lop: " << maLop << " - " << dsLopHoc[idxLop].tenLop << endl;
  ...
  for (int i = 0; i < svTrongLop.size(); i++) {
    ...
    for (int j = 0; j < dsDiemDanh.size(); j++) {
      if (dsDiemDanh[j].maLop == maLop && dsDiemDanh[j].maSV == maSV) {
        if (dsDiemDanh[j].trangThai == CO_MAT)
          coMat++;
        else if (dsDiemDanh[j].trangThai == VANG_CO_PHEP)
          vangCP++;
  ...
  cout << endl
       << "  Ghi chu: CM=Co mat, VCP=Vang co phep, VKP=Vang khong phep" << endl;
...
void HeThongDiemDanh::thongKeSiSo() {
  ...
  for (int n = 0; n < ngays.size(); n++) {
    ...
    double tiLeCM =
        (tongDiemDanh > 0) ? (double)coMat / tongDiemDanh * 100.0 : 0;
  ...
  cout << endl
       << "  Tong cong: " << ngays.size() << " buoi da diem danh." << endl;
  nhanEnter();
}
...
void HeThongDiemDanh::danhSachVangNhieuNhat() {
  ...
  cout << endl
       << "  Hien thi " << soHienThi << "/" << dsThongKe.size()
